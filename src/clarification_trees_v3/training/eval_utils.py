...
def plot_stacked_metrics(df_inf: pd.DataFrame, df_qp: pd.DataFrame, df_ent: pd.DataFrame, output_dir: Path):
    """
    Generates and saves pointplots for Depth vs. Scores, with lines for each iteration stacked on the same graph.
    Assumes the DataFrames contain an 'Iteration' column.
    """
    output_dir.mkdir(parents=True, exist_ok=True)
    sns.set_theme(style="whitegrid")
    ...
    for df, ylabel, filename, title, y_lim in datasets:
        if df.empty or 'Iteration' not in df.columns:
            print(f"Skipping {title} - no data or missing 'Iteration' column.")
            continue
